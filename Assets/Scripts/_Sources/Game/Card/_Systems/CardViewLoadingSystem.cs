...
namespace Game
{
	public class CardViewLoadingSystem : ReactiveSystem<CardEntity>, IInitializeSystem, ITearDownSystem
	{
		private CardContext _context;
		private CardSetting _setting;
		private List<GameObject> _linkedObjects;

		public CardViewLoadingSystem(Contexts contexts, CardSetting setting) : base(contexts.card)
		{
		...
		{
			foreach(var e in _context.GetEntities(CardMatcher.GameCard))
			{
				if(Filter(e))
				{
					LoadResource(e);
		...
		{
			return context.CreateCollector(CardMatcher.GameCard);
		}
